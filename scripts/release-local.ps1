...
$VpkVersion = '1.1.1'               # NuGet パッケージ側の Velopack (IRUZ.csproj) と同一バージョンに固定
$WranglerVersion = '4.92.0'         # サプライチェーン対策でバージョン固定
$Bucket = 'iruz-updates'
$BaseUrl = 'https://iruz.nephilim.jp'
...
# IRUZ の配信 RID は win-x64 のみ (channel = win)
$RuntimeMatrix = @{
    'win-x64' = @{ Channel = 'win' }
}
...
# アイコン生成 (CI 版と同じく明示実行。csproj の BeforeBuild ターゲットでも走るが冪等)
Invoke-Native 'アイコン生成' { pwsh -NoProfile -File (Join-Path 'icon' 'generate_icon.ps1') }
