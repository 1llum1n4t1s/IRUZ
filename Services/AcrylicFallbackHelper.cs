...
/// </summary>
public static class AcrylicFallbackHelper
{
    ...
    /// <summary>
    /// 判定ロジック本体（テスト用に純粋関数として分離）。
    /// </summary>
    /// <param name="acrylicGranted">アクリルが適用済みと報告されているか。</param>
    /// <param name="isWindows">Windows 上で動作しているか。</param>
    /// <param name="isRemoteSession">リモートデスクトップセッションか。</param>
    /// <param name="transparencyEnabled">OS の透過効果が有効か。</param>
    /// <returns>不透明背景へフォールバックすべきなら true。</returns>
    public static bool ShouldUseOpaqueBackgroundCore(
